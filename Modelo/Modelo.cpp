...
//tengo que remover al mario agregado en el primer nivel
this->colisionador.desconectarJugador("Jugador1");
...
void Modelo::agregarObjetosAlColisionador(){
    for(int i=0; i < modelosPersonajes.size(); i++){
        ...
        if(this->modelosPersonajes[i]->getNombre() == "mario"){
            this->colisionador.agregarJugador("Jugador1",this->modelosPersonajes[i]);
        }
